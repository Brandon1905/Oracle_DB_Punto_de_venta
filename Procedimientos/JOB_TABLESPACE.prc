...
begin
  ...
  FOR SED IN (select U.CORREO
    ...
    send_mail_test.mail_test(msg_to      => SED.CORREO,
                             msg_subject => 'TABLESPACE CON UN 85% DE SU TAMAÑO',
                             msg_text    => AUX);
  END LOOP;

  -- Verificar el tamaño de los tablespace, el cual no debe de exceder en un 85% su tamaño. Este proceso se 
  -- ejecutará todos los días, notificar en caso de que exista inconsistencia
